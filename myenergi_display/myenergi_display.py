...
class MyEnergi(object):
    ...
    def _exec_api_cmd(self, url):
        ...
        print(f"PJA: response.status_code = {response.status_code}")
        if response.status_code != 200:
            raise Exception(f"{response.status_code} error code returned from myenergi server.")
        response_dict = response.json()
# PJA        print(f"PJA: response_dict = {response_dict}")
    ...
    def set_zappi_charge_schedule(self, charge_slot_dict_list):
        """@brief Set the charge schedule for the zappi.
           @param charge_slot_dict_list A list of dicts holding the start stop details of the charge as generated by
                                   GUIServer._set_zappi_charge_thread()."""
        print(f"PJA: len(charge_slot_dict_list)={len(charge_slot_dict_list)}")
...
#            print(f"PJA: self._other_tariff_values={self._other_tariff_values}")
        except Exception:
...
#                    break
    ...
    #            print(f"PJA: time_interval={time_interval}, price = {price}")
...
# PJA REMOVE
#        with ui.row().classes('w-full'):
#            ui.label('Charge end time')
#            self._end_charge_time_input = ui.input('Time')
#            with self._end_charge_time_input as time:
...
#                    ui.icon('access_time').on('click', menu.open).classes('cursor-pointer')
    ...
    def _set_zappi_charge_thread(self):
        for charge_slot_dict in self._charge_slot_dict_list:
            print(f"PJA: charge_slot_dict={charge_slot_dict}")
        ...
        for sorted_charge_slot_dict in sorted_charge_slot_dict_list:
            print(f"PJA: sorted_charge_slot_dict={sorted_charge_slot_dict}")
        ...
        try:
            for merged_charge_slot_dict in merged_charge_slot_dict_list:
                print(f"PJA: merged_charge_slot_dict={merged_charge_slot_dict}")
    ...
    def _reset_zappi_charge_thread(self):
        ...
        try:
            ...
            msg_dict = {}
            msg_dict[GUIServer.INFO_MESSAGE] = "Cleared all zappi charging schedules."
            self._update_gui(msg_dict)
        except Exception as ex:
            msg_dict = {}
            msg_dict[GUIServer.ERROR_MESSAGE] = str(ex)
